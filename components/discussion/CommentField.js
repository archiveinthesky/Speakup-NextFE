...
const CommentField = ({ boardId, onSide }) => {
  ...
  const fetchCommentReply = async (commentId) => {
    ...
    let cmtarray = [];
    let alreadyExistedIDs = newComments[targetCmtPos].replies.newReplies.map(
      (reply) => {
        return reply.id;
      }
    );
        ...
        <div className="bg-neutral-50">
          ...
          <div className="mx-auto mb-4 flex flex-col divide-y divide-neutral-300 px-9 py-3 ">
            {[...userComments, ...comments].map((cmt, i) => {
              if (i === 1) {
                console.log(
                  cmt.replies.latestServerReply + cmt.replies.newReplies.length
                );
                console.log(cmt.replies.hasReplies);
              }
              return (
