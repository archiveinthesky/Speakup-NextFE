const getComments = async (start, end, boardId, onSide) => {
    ...
    return await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/comments/${boardId}/${onside}`, {
        method: 'GET',
        headers: {
...
const getCommentReply = async (commentId, start, end, boardId, onSide) => {
    let onside = ['sup', 'all', 'agn'][onSide]
    return await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/comments/${boardId}/${onside}/${commentId}`, {
        method: 'GET',
        headers: {
            'Authorization': localStorage.getItem("AuthToken"),
            'Accept': 'application/json',
...
export { getComments, getCommentReply, postReplyBE }